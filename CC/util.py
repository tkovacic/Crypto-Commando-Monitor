...
def printInterface(market, marketCoin, marketFiat, tv, dl, pp, cp, yp, ldl, lb, ls, profits, coin, tmpDelta, client):
    try:
        ...
        if(str(config.COIN_ID) != "" and str(config.FIAT_ID) != ""):
            print("Holding: " + str(round(float(client.get_account(config.COIN_ID)["balance"]),6)) + " " + marketCoin + " / " + str(round(float(client.get_account(config.FIAT_ID)["balance"]),2)) + " " + marketFiat + " || Total: " + str(round((round(float(client.get_account(config.COIN_ID)["balance"]),2) * float(cp)) + round(float(client.get_account(config.FIAT_ID)["balance"]),2),2)) + " " + marketFiat);
        print("");
